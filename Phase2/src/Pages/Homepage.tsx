...
function Homepage() {
  ...
  const inputStyle = {
    "& .MuiInputBase-input": { color: "#ECDFCC" },
    "& .MuiOutlinedInput-notchedOutline": { borderColor: "#ECDFCC" },
    "&:hover .MuiOutlinedInput-notchedOutline": { borderColor: "#ECDFCC" },
  ...
  const fetchSearchResults = async (query: string) => {
    ...
    try {
      ...
      setApiResults([...movieRes.data.results, ...tvRes.data.results]);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
      ...
      <Box sx={{ background: "#181C14", pt: 10 }}>
        ...
        {searchText !== "" ? (
          <Box sx={{ p: 2 }}>
            ...
            {loading ? (
              <Typography sx={{ color: "#ECDFCC" }}>Loading...</Typography>
            ) : apiResults.length > 0 ? (
              <Box
                sx={{
                  display: "grid",
